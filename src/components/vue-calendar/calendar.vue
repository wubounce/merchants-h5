...
.calendar .isGregorianFestival,
.calendar .isLunarFestival{
    color:#ea6151;
}
.calendar-years{
    height: 5.6rem;
...
<template>
    <div class="calendar">
        <div class="calendar-tools" v-if="type==='datetime'">
        ...
        <div class="months-calendar-tools" v-if="type==='month'">
            <div class="year" @click.stop="changeYear">{{year}}</div>
        </div>
        <section class="date-wrap"  v-if="type==='datetime'">
        ...
        <div class="calendar-months" v-if="type==='month'">
            <div  v-for="(m,index) in multiMonths"   @click.stop="selectmultiMonth(m.monthNum)"  :class="{'selected':m.selected}">
                <span>{{m.month}}</span>
            </div>
        </div>
...
export default {
    ...
    methods: {
            ...
            if (this.value.length>0) {
                if (this.range) { //范围
                    if (this.type==='datetime') {
                        ...
                        this.rangeBegin = [this.year, this.month];
                        this.rangeEnd = [year2, month2]; 
                    }
        ...
        selectmultiMonth(value){
            this.selectultiMonth=value;
            this.multiMonths.forEach(i=>{
                i.selected = false;
            });
            if (this.range) {
                if (this.rangeBegin.length === 0 || this.rangeEndTemp !== 0) {
                    this.rangeBegin = [this.year, this.selectultiMonth];
                    this.rangeBeginTemp = this.rangeBegin;
                    this.rangeEnd = [this.year, this.selectultiMonth];
                    this.rangeEndTemp = 0;
                    ...
                    this.rangeEndTemp = 1;
                        // 判断结束日期小于开始日期则自动颠倒过来
                    if (+new Date(this.rangeEnd[0], this.rangeEnd[1]) < +new Date(this.rangeBegin[0], this.rangeBegin[1])) {
                        this.rangeBegin = this.rangeEnd;
                        this.rangeEnd = this.rangeBeginTemp;
                    }
                    // 小于10左边打补丁
