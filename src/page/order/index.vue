          ...
          <div class="ios">
              <div class="alllist" v-for="(item,index) in list" :key="index">
                  ...
                  <section class="listaction" v-if="item.orderStatus === 2"> 
                      ...
                      <mt-button @click="machineReset(item.orderNo,item.machineId,item.machineName)"  v-if="item.isESource === 0 || item.isESource === null || item.isDrinking === 0" v-has="'mer:order:reset,mer:order:info'">复位</mt-button>
                  </section>
                </div>
            </div>
